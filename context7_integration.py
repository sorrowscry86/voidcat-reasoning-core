...
class Context7Engine:
    ...
    async def _initialize_context_sources(self) -> None:
        ...
        if self.context_sources:
            ...
            debug_print(f"Context7 initialization complete: {total_sources} sources loaded")
        else:
    ...
    def _extract_document_metadata(self, content: str, file_path: Path) -> Dict[str, Any]:
        ...
        if tag_matches:
            ...
            for match in tag_matches:
                tags.extend([tag.strip() for tag in re.split(r'[,\s]+', match) if tag.strip()])
    ...
    async def _build_source_vectors(self) -> None:
        ...
        try:
            ...
            debug_print(f"Built vectors for {len(documents)} sources with {self.source_vectors.shape[1]} features")
        except Exception as e:
    ...
    async def _analyze_context_clusters(self) -> None:
        ...
        try:
            ...
            for i, source in enumerate(source_list):
                ...
                similar_sources = [source_list[j] for j in similar_indices 
    ...
    async def retrieve_context(self, request: ContextRequest) -> ContextResponse:
        ...
        try:
            ...
            processing_metadata = {
                ...
                "max_relevance_score": max(relevance_scores.values()) if relevance_scores else 0,
                "clusters_analyzed": len(self.context_clusters),
                "processing_strategy": request.strategy
            }
            
            debug_print(f"Context retrieval: {len(selected_sources)} sources selected from {len(self.context_sources)} available")
    ...
    async def _calculate_relevance_score(self, query: str, source: ContextSource) -> float:
        """
        ...
        try:
            ...
            if source.name.lower() in query_lower or any(word in source.name.lower() for word in query_lower.split()):
                metadata_boost += 0.2
    ...
    async def ensure_initialized(self) -> None:
        ...
        if not self._initialized and not self._initialization_task:
            self._initialization_task = asyncio.create_task(self._initialize_context_sources())
    ...
    def get_diagnostics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "vectorizer_features": self.source_vectors.shape[1] if self.source_vectors is not None else 0,
