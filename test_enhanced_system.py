...
async def test_context7_engine():
    ...
    if result.sources:
        for i, source in enumerate(result.sources[:2], 1):
            print(f"  Source {i}: {source.name} (relevance: {result.relevance_scores.get(source.id, 0):.3f})")
...
async def test_enhanced_engine():
    ...
    try:
        ...
        print(f"  Contains reasoning analysis: {'Sequential Reasoning Analysis' in response}")
        print()
    ...
    try:
        ...
        print(f"  Total queries processed: {enhanced_stats.get('total_queries_processed', 0)}")
        print(f"  Enhanced queries: {enhanced_stats.get('enhanced_queries_processed', 0)}")
