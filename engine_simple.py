#!/usr/bin/env python3
"""
VoidCat Reasoning Core Engine - Simplified Clean Version
Reliable MCP server compatible engine with basic RAG functionality
"""
...
class VoidCatEngine:
    ...
    def __init__(self, knowledge_dir: str = "knowledge_source"):
        ...
        if not self.openai_api_key and not self.deepseek_api_key:
            debug_print(
                "Warning: No API keys found. Please set OPENAI_API_KEY or DEEPSEEK_API_KEY"
            )
    ...
    def _load_documents(self, knowledge_dir: str) -> None:
        ...
        if self.documents:
            try:
                ...
                debug_print(
                    f"✓ Vectorized {len(self.documents)} documents with {self.doc_vectors.shape[1]} features"
                )
            except Exception as e:
