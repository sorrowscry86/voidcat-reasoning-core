...
class VoidCatEnhancedEngine:
    ...
    def _load_documents(self, knowledge_dir: str) -> None:
        ...
        if not os.path.isdir(knowledge_dir):
            debug_print(
                f"Warning: Knowledge directory '{knowledge_dir}' not found"
            )
            return
    ...
    async def query(self, user_query: str, model: str = "gpt-4o-mini", top_k: int = 2, session_id: str = None) -> str:
...
Please provide a detailed, well-reasoned response based on the context."""
        ...
        response = await self._call_api(
            enhanced_prompt, model, api_key, api_url
        )
    ...
    def get_diagnostics(self) -> dict:
        ...
        return {
            ...
            "api_configured": bool(
                self.openai_api_key or self.deepseek_api_key
            ),
        }
